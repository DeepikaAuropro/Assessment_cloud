...
resource "aws_cloudfront_distribution" "cdn" {
  depends_on = [aws_s3_bucket.bucket] # Ensuring the S3 bucket is created before CloudFront
...
resource "aws_lb_target_group" "asg_target_group" {
  ...
  port     = 80  # Port on which ASG instances will receive traffic
  protocol = "TCP"
  vpc_id   = aws_vpc.main.id
}
...
resource "aws_lb_listener" "listener" {
  ...
  port              = 80  # Port on which NLB listens for incoming traffic
  protocol          = "TCP"
...
# Security Group Rule for NLB Ingress
resource "aws_security_group_rule" "nlb_ingress" {
  type              = "ingress"
  from_port         = 80
  to_port           = 80
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]  # Adjust this to your actual IP range for CloudFront access
